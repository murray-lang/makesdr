...
QtBandReadout::QtBandReadout(IRadioSettingsUpdater* radioControl, SplitBandId bandIndex, QWidget* parent)
  : QWidget(parent)
...
void QtBandReadout::buildUi()
{
  ...
  // --- VFO A
  m_vfoA = new QtVfoReadout(m_radioControl, VfoId::A, m_leftCell);
  ...
  m_vfoB = new QtVfoReadout(m_radioControl, VfoId::B, m_rightStackHost);
...
void QtBandReadout::applyFrequencyChanges(const RxTxDualIqBandSettings* bandSettings, bool onlyIfChanged)
{
  if (!bandSettings) return;

  applyFrequencyChanges(m_vfoA, bandSettings->pipeline(VfoId::A), onlyIfChanged);
  applyFrequencyChanges(m_vfoB, bandSettings->pipeline(VfoId::B), onlyIfChanged);
}
                                         ...
                                         bool onlyIfChanged)
{
  ...
  if (rfSettings.hasFrequency() || !onlyIfChanged) {
    readout->setValue(rfSettings.frequency());
  }
}

void QtBandReadout::applyPipelineChanges(const RxTxDualIqBandSettings* bandSettings, bool onlyIfChanged)
{
  if (!bandSettings) return;

  const auto applyMode = [onlyIfChanged](QtVfoReadout* readout, const RxPipelineSettings* pipeline) {
    if (!readout) return;
    ...
    if (onlyIfChanged && !pipeline->base().isMode()) {
      return;
    }

    readout->setModeText(QString::fromStdString(pipeline->base().mode().name().c_str()));
  };

  applyMode(m_vfoA, bandSettings->pipeline(PipelineId::A));
  applyMode(m_vfoB, bandSettings->pipeline(PipelineId::B));
}
