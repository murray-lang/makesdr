...
public:
  ...
  [[nodiscard]] int targetMajorPx() const noexcept { return m_targetMajorPx; }

  // If true, include endpoints (min/max) as major ticks.
  void setIncludeEndpoints(bool on) { m_includeEndpoints = on; }
  [[nodiscard]] bool includeEndpoints() const noexcept { return m_includeEndpoints; }

  // Minor ticks per major interval.
  void setMinorsPerMajor(int n) { m_minorsPerMajor = std::clamp(n, 0, 50); }
  [[nodiscard]] int minorsPerMajor() const noexcept { return m_minorsPerMajor; }

  // Labels
  void setLabelMode(LabelMode m) { m_labelMode = m; }
  [[nodiscard]] LabelMode labelMode() const noexcept { return m_labelMode; }

  // Numeric formatting
  void setDecimals(int decimals) { m_decimals = decimals; }
  [[nodiscard]] int decimals() const noexcept { return m_decimals; }

  void setPrefix(const QString& s) { m_prefix = s; }
  [[nodiscard]] const QString& prefix() const noexcept { return m_prefix; }

  void setSuffix(const QString& s) { m_suffix = s; }
  [[nodiscard]] const QString& suffix() const noexcept { return m_suffix; }

  // Priorities
  void setEndpointPriority(int p) { m_endpointPriority = p; }
  [[nodiscard]] int endpointPriority() const noexcept { return m_endpointPriority; }

  void setMajorPriority(int p) { m_majorPriority = p; }
  [[nodiscard]] int majorPriority() const noexcept { return m_majorPriority; }
...
private:
  ...
  [[nodiscard]] QString formatValue(double v, double chosenMajorStep) const;
