...
GpioLines::GpioLines(const GpioLineMask& lines) :
  m_lines(lines),
  m_direction(Direction::AS_IS),
  m_bias(Bias::AS_IS),
  m_edge(Edge::NONE)
...
{
  GpioLines::configureLines(config);
}

GpioLines::GpioLines(const GpioLineMask& lines, const Config::GpioLines::Fields& config) : GpioLines()
{
  GpioLines::configureLines(config);
  m_lines = lines;
}

GpioLines::GpioLines(const GpioLineMask& lines, Direction direction, Bias bias, Edge edge) :
  m_lines(lines),
  m_direction(direction),
  m_bias(bias),
  m_edge(edge)
...
{
  using string = Config::GpioLines::ConfigString;

  const string& dir = config.direction;
  if (dir == "input") {
