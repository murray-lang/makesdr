...
ResultCode LibUsbControl::initialise() {
    ...
    if (init_rc != LIBUSB_SUCCESS) {
       return ResultCode::ERR_SETTING_CONTROL_USB_LIB_INIT;
    }
...
ResultCode LibUsbControl::open() {
    ...
    libusb_free_device_list(device_list, 1);
    if (rc < 0) {
      return ResultCode::ERR_SETTING_CONTROL_USB_LIB_OPEN;
    }
    rc = libusb_detach_kernel_driver(handle,0);
    if(rc < 0) {
      return ResultCode::ERR_SETTING_CONTROL_USB_LIB_DETACH_KERNEL_DRIVER;
    }
    rc = libusb_claim_interface(handle,0);
    if(rc < 0) {
      return ResultCode::ERR_SETTING_CONTROL_USB_LIB_CLAIM_INTERFACE;
    }
  return ResultCode::OK;
}
