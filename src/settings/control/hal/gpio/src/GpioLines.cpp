...
GpioLines::GpioLines(const GpioLineNumberVector& lines) :
  m_lines(lines),
  m_direction(Direction::AS_IS),
  m_bias(Bias::AS_IS),
  m_edge(Edge::NONE)
...
GpioLines::GpioLines(const GpioLineNumberVector& lines, const Config::GpioLines::Fields& config) : GpioLines()
{
  configureLines(config);
  m_lines = lines;
}

GpioLines::GpioLines(const GpioLineNumberVector& lines, Direction direction, Bias bias, Edge edge) :
  m_lines(lines),
  m_direction(direction),
  m_bias(bias),
  m_edge(edge)
